...
import { Users, DollarSign, Briefcase, CalendarCheck, TrendingUp, ArrowRight, Activity, Bell } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const StatCard = ({ title, value, icon: Icon, colorClass, trend, link }: any) => (
    <div 
        ...
        className="group relative bg-white rounded-2xl p-6 shadow-card hover:shadow-card-hover transition-all duration-300 border border-slate-100 cursor-pointer overflow-hidden"
    >
      ...
      <div className="flex items-start justify-between relative z-10">
          ...
          <div className={`p-3 rounded-xl bg-gradient-to-br ${colorClass} text-white shadow-lg shadow-primary-500/20 group-hover:scale-110 transition-transform`}>
            <Icon size={22} />
          </div>
      </div>
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="flex items-center gap-3">
             <button className="flex items-center gap-2 bg-white text-slate-600 px-4 py-2 rounded-xl border border-slate-200 shadow-sm hover:bg-slate-50 text-sm font-semibold">
                <CalendarCheck size={16} /> {new Date().toLocaleDateString()}
             </button>
             <button className="bg-primary-600 text-white px-4 py-2 rounded-xl shadow-lg shadow-primary-500/30 hover:bg-primary-700 text-sm font-semibold transition-transform active:scale-95">
                Download Report
             </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-8 rounded-3xl border border-slate-100 shadow-card relative overflow-hidden">
          ...
          <div className="h-64 w-full flex items-end justify-center gap-16 md:gap-32 p-4 relative">
             ...
             <div className="flex flex-col items-center gap-3 group w-24">
                 <div className="text-sm font-bold text-slate-700 bg-white shadow-sm border px-3 py-1 rounded-lg mb-2 opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0">{currency}{stats.collectedFees.toLocaleString()}</div>
                 <div className="w-full bg-gradient-to-t from-emerald-500 to-emerald-400 rounded-2xl shadow-lg shadow-emerald-500/20 transition-all duration-700 group-hover:scale-105" style={{ height: `${collectedHeight}%`, minHeight: '8px' }}></div>
                 <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">Collected</div>
             </div>
             
             <div className="flex flex-col items-center gap-3 group w-24">
                 <div className="text-sm font-bold text-slate-700 bg-white shadow-sm border px-3 py-1 rounded-lg mb-2 opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0">{currency}{stats.pendingFees.toLocaleString()}</div>
                 <div className="w-full bg-gradient-to-t from-slate-300 to-slate-400 rounded-2xl transition-all duration-700 group-hover:scale-105" style={{ height: `${pendingHeight}%`, minHeight: '8px' }}></div>
        ...
        <div className="space-y-6">
            <div className="bg-gradient-to-br from-indigo-900 to-slate-900 p-6 rounded-3xl shadow-xl text-white relative overflow-hidden">
                ...
                <div className="space-y-3 relative z-10">
                    <button onClick={() => navigate('/students')} className="w-full text-left px-4 py-3 rounded-xl bg-white/10 hover:bg-white/20 border border-white/5 backdrop-blur-sm text-sm font-semibold transition-colors flex items-center justify-between group">
